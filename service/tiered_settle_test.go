...
func TestPrepareTieredBillingForSelectedGroupRejectsInsufficientWalletTopUp(t *testing.T) {
	truncate(t)

	const userID = 701
	// The initial 50k reservation has already been deducted. The remaining
	// balance cannot cover the retry group's additional 50k reservation.
...
func TestBillingSessionSettleRejectsInsufficientWalletTopUp(t *testing.T) {
	truncate(t)
	...
	err := session.Settle(80_000)
	require.ErrorIs(t, err, model.ErrInsufficientUserQuota)

	userQuota, queryErr := model.GetUserQuotaFromDB(userID)
