...
| GG-002 | docs | `.agents/skills/ggapi-fork/`；`docs/fork/README.md` / `AGENTS.md` 入口链接（含 GG-003 runner 口径：SKILL 摘要表、checklists、troubleshooting §18 均随 workflow 一同改为 GitHub 托管） | `/ggapi-fork` 教练 skill **v1.7.0**（Modes A–I + 可控自升级 + C2-pre 禁自跳 + **C-continue** + 链式 C→F + 第三壳 + **tag `v<上游>.N` → GHCR wait**）；含 oxlint 发车、merge flake 校验、GitHub 托管 runner、检查更新 PAT、i18n zh-TW 等 | 二开需要命令级教练，且 skill 须能跟进文档/实战而不静默削弱安全规则 | low（独立 skill 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | `/ggapi-fork` 可发现；默认前端 `web/ggapi`；tag→GHCR 见 SOP §5.1 / GG-003；C2-pre 可走通且修完再审；PR 已开时「继续」不重写代码且不补 push 词；链式发车缺词不补权；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-*` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `docs/ggapi-fork-1.6.0` / `docs/ggapi-fork-1.7.0` |
| GG-003 | infra | `.github/workflows/*`（`ci.yml`、`docker-build.yml`、`docker-image-branch.yml`、`release.yml`、`electron-build.yml`） | CI **GitHub 托管 runner**（`ubuntu-latest`）+ **Linux amd64**；**PR/main 门禁 `ci.yml`**（后端 stub 三壳 dist 后 `go build` + `go test ./...`；前端按 matrix 构建 ggapi/default/classic 三壳，ggapi/default 另跑 typecheck，均为硬门禁；ggapi/default 另跑 `bun test`（classic 无测试文件）；不跑 oxlint/format/copyright（基线已有报错，只能非阻塞，而永不失败的步骤给不出信号）。跑在托管 runner 的一次性 VM 上，fork PR 执行自身代码属该模型常态，workflow 内无需自设拦截（也拦不住——PR 跑的是自己那份文件）；后端先按 `go.mod` 声明的下限编译（含测试二进制），再以 `CGO_ENABLED=0` + `GOEXPERIMENT=greenteagc` + **从 Dockerfile builder2 读出的**工具链编译并测试，版本不在此处二次硬编码，checkout 不持久化凭据，job 设 timeout，push main 不被取消；Go 双档（`go.mod` 下限编译 + 发版工具链编译测试，两档均设 `GOTOOLCHAIN=local` 防止自动切换）、bun 钉死小版本（与 Dockerfile 的 digest 同为手动升级，两处需一起改）；default 壳按 Dockerfile builder 不加 --filter 安装）；**tag → GHCR** 仅 `:<version>`（tip 时再加 `:latest`），无冗余 `*-amd64` 正式 tag；`cache-to` **ignore-error**；+ **Release 元数据**；**禁止** Docker Hub `calciumion/new-api`；裸二进制仅 `release.yml` **workflow_dispatch**；`release.yml` ldflags 包路径修正为 `github.com/QuantumNous/new-api/common.Version`（原 `new-api/...` 不匹配模块名，`-X` 静默失效，裸二进制版本号从未被注入）；electron 构建对本仓禁用 | GitHub 托管 runner（公开仓免费额度）；部署走镜像；合入前需自动验证后端测试与产品壳可构建；更新检查依赖 `releases/latest`；避免污染上游 Hub；cache 故障不拖垮已 push 镜像 | medium（上游常改 workflow；`sync/upstream-20260713` 冲突保留本仓 GHCR 策略） | 推 fork tag → 仅 `IMAGE:TAG`（+ tip 时 `latest`）+ Release；不默认 bare 二进制；branch 镜像 `branch-*-hash`；上游同步后仍为 `ghcr.io/...`；PR 触发 `ci.yml` 的 backend 与 frontend(ggapi/default/classic) 均绿；fork PR 在托管 runner 上正常运行；lint/format/copyright 检查尚无门禁，属已知范围外；构建后断言 `dist/index.html` 存在；push main 的跑不被后续合并取消 | active | Akuma-real | 2026-07-11 | `chore/org-linux-runner` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `fix/ghcr-cache-ignore-error-single-tags` / `sync/upstream-20260713` / `chore/ci-pr-gate` |
