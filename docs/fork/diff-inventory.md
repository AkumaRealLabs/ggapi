...
| GG-002 | docs / infra | `.agents/skills/ggapi-fork/`；`.codex/hooks.json`；`.claude/settings.json`；`.gitignore`（仅放行 Claude 项目 Hook）；`docs/fork/README.md` / `AGENTS.md` 入口链接 | `/ggapi-fork` 教练 skill **v1.9.0**（Modes A–I + 可控自升级 + Codex/Grok/Claude 原生运行时探测与项目 Hook + **C2-pre 各环境用自家 reviewer**（Codex→`codex review`、Claude→`claude -p "/code-review origin/main"`、Grok→`grok -p` headless；缺 CLI 回退 Codex CLI）+ C2-pre 精确树记录/失效 + 禁自跳 + **C-continue** + 链式 C→F + 第三壳 + **tag `v<上游>.N` → GHCR wait**）；含 oxlint 发车、merge flake 校验、org-linux、检查更新 PAT、i18n zh-TW 等 | 二开需要命令级教练并自动使用当前 Agent 的原生能力（含各环境自己的 code review），不能把 Grok plugin 路径当成所有环境的固定入口；skill 须跟进文档/实战且不静默削弱安全规则 | low（独立 skill/Hook 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | `/ggapi-fork` 可发现；`agent-adapter.mjs detect` 在 Codex/Grok/Claude 各选自家 reviewer（`gate-record --reviewer` 记实际执行者，fallback 记 codex）；Codex/Claude Hook schema 有效，Grok `inspect` 能发现兼容 Hook；未审查 commit/push/merge/tag 被拒；clean/bypass/empty 放行；同树 commit 不失效，内容/分支/base 变化失效；复合/动态发车命令被拒，`git -C/-c` 仍可识别，push/merge/tag 按实际 source/ref/pinned head tree 校验；`.claude` 仍默认忽略且仅跟踪 `settings.json`；默认前端 `web/ggapi`；tag→GHCR 见 SOP §5.1 / GG-003；PR 已开时「继续」不重写代码且不补 push 词；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-*` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `docs/ggapi-fork-1.6.0` / `docs/ggapi-fork-1.7.0` / `docs/ggapi-fork-native-adapters` |
