...
| GG-005 | feature / infra | `web/ggapi/**`；`web/package.json` workspaces；`makefile`（`build-web-ggapi` / `dev-web-ggapi`，`dev`→ggapi）；`Dockerfile` `builder-ggapi`；`main.go` embed；`common/constants.go` / `embed-file-system.go`；`router/web-router.go`；`setting/system_setting/theme.go`（默认 `ggapi`）；`controller/option.go` 校验；`AGENTS.md` / `docs/fork/*` 第三壳与追 default 说明 | 第三前端壳 `web/ggapi`（自 default 复制起步）；运行时 `theme.frontend=ggapi`；构建/embed/Docker 全链路；**功能长期追 `web/default`**，视觉与产品定制落在 ggapi | 避免在 `web/default` 上做永久大改导致上游同步痛苦；独立壳便于皮肤与产品化 | high（三壳源码树 + 多处接线；上游改 theme/embed/Docker/makefile 时需重放） | `theme.frontend` 可选 `ggapi\|default\|classic`；默认 ggapi；`make build-web-ggapi` + `go build` embed 通过；Docker 三 stage 产物齐全；同步后 default 新功能有 port 到 ggapi 的 follow-up | active | Akuma-real | 2026-07-11 | `feat/web-ggapi-shell` |
