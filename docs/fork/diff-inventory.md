...
| GG-002 | docs | `.agents/skills/ggapi-fork/`；`docs/fork/README.md` / `AGENTS.md` 入口链接 | `/ggapi-fork` 教练 skill **v1.4.0**（Modes A–I + 可控自升级 + **最终提交前 Codex 审查环** + **第三壳 GG-005 日常/发车教练**）；含 org-linux/CI、检查更新 PAT、`gh` 目标仓、i18n zh-TW、release embed 等实战条目 | 二开需要命令级教练，且 skill 须能跟进文档/实战而不静默削弱安全规则 | low（独立 skill 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | `/ggapi-fork` 可发现；默认前端路径 `web/ggapi`；提交前 Codex 门禁可走通（或明确跳过）；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-self-upgrade` / `docs/ggapi-fork-1.2.0` / `docs/ggapi-fork-1.3.0` / `docs/ggapi-fork-1.4.0` |
