...
| GG-002 | docs | `.agents/skills/ggapi-fork/`；`docs/fork/README.md` / `AGENTS.md` 入口链接 | `ggapi-fork` 教练 skill **v1.8.0**（Codex `$ggapi-fork` / Grok `/ggapi-fork`；Modes A–I + 可控自升级 + **Codex App / Codex CLI / Grok Build CLI** 多运行面 C2-pre + 禁自跳 + **C-continue** + 链式 C→F + 第三壳 + **tag `v<上游>.N` → GHCR wait**）；含当前消息授权、Ship Status Contract、oxlint 发车、merge flake 校验、org-linux、检查更新 PAT、i18n zh-TW 等 | 二开需要命令级教练，且 skill 须能跟进文档/实战而不静默削弱安全规则 | low（独立 skill 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | Codex App / CLI 可通过 Skills、`$ggapi-fork` 或自然语言发现，Grok 可用 `/ggapi-fork`；默认前端 `web/ggapi`；tag→GHCR 见 SOP §5.1 / GG-003；C2-pre 在三种运行面均可走通且修完再审；PR 已开时「继续」只报告状态不补 push/PR/merge 权限；链式发车缺词不补权；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-*` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `fix/workflow-guardrails` |
| GG-003 | infra | `.github/workflows/*`（`ci.yml`、`pr-check.yml`、`docker-build.yml`、`docker-image-branch.yml`、`release.yml`、`electron-build.yml`） | CI 统一 `org-linux` + **Linux amd64**；PR 使用 `pull_request`，编译 root/router、运行 scoped Go tests，并执行三壳质量与条件 full-shell build；tag → GHCR 仅 `:<version>`（tip 时再加 `:latest`），无冗余 `*-amd64` 正式 tag；`cache-to` **ignore-error**；+ **Release 元数据**；**禁止** Docker Hub `calciumion/new-api`；裸二进制仅 `release.yml` **workflow_dispatch**；electron 构建对本仓禁用 | 组织 runner；PR 代码质量和部署走同一组织 runner；更新检查依赖 `releases/latest`；避免污染上游 Hub；cache 故障不拖垮已 push 镜像 | medium（上游常改 workflow；`sync/upstream-20260713` 冲突保留本仓 GHCR/PR CI 策略） | PR head 变化自动重跑；root/router 可编译；按改动路径输出明确 success/skip；推 fork tag → 仅 `IMAGE:TAG`（+ tip 时 `latest`）+ Release；不默认 bare 二进制；branch 镜像 `branch-*-hash`；上游同步后仍为 `ghcr.io/...` | active | Akuma-real | 2026-07-11 | `chore/org-linux-runner` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `fix/ghcr-cache-ignore-error-single-tags` / `sync/upstream-20260713` |
...
| GG-008 | infra / docs | `.codex/hooks.json`；`.codex/hooks/`；`.codex/rules/ggapi.rules`；`.githooks/`；`makefile`；`.github/workflows/ci.yml`；`.github/workflows/pr-check.yml`；`.github/PULL_REQUEST_TEMPLATE.md`；`docs/fork/codex-workflow-guardrails.md`；`AGENTS.md`；`.agents/skills/ggapi-fork/**` | 新增跨运行面工作流守卫：统一阶段与当前消息授权、完整 Ship Status；Codex App/CLI 项目 Hooks；execpolicy upstream/main/`--no-verify`/常见 `core.hooksPath` 绕过禁止与 ship 动作 prompt；所有客户端共用 origin 白名单 Git Hooks；PR pull_request 分层 CI 与 stale review 说明 | 同一 L2 变更可能从 Codex App、Codex CLI、Grok Build CLI 或终端进入，必须防止“提交后自动推送/PR/合并”的权限漂移，同时让私有仓 CI 可见且诚实 | medium（独立守卫文件；`AGENTS.md`、workflow、makefile 为旁路接线） | Node `node:test` 覆盖 main/topic、dirty/ahead/behind、remote 脱敏、成功/失败阶段、release、已提交 inventory、Stop/review 隔离；`codex execpolicy check` 覆盖 forbidden/prompt/allow 与常见 hook 绕过；Git Hook 覆盖 origin URL 白名单与 main 拒绝；PR head synchronize 重跑；未启用 branch protection 时不宣称服务端强制 | active | Akuma-real | 2026-07-18 | `docs/ggapi-fork-multi-surface-review` |
