...
| GG-002 | docs | `.agents/skills/ggapi-fork/`；`docs/fork/README.md` / `AGENTS.md` 入口链接 | `/ggapi-fork` 教练 skill **v1.7.0**（Modes A–I + 可控自升级 + C2-pre 禁自跳 + **C-continue** + 链式 C→F + 第三壳 + **tag `v<上游>.N` → GHCR wait**）；含 oxlint 发车、merge flake 校验、org-linux、检查更新 PAT、i18n zh-TW 等 | 二开需要命令级教练，且 skill 须能跟进文档/实战而不静默削弱安全规则 | low（独立 skill 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | `/ggapi-fork` 可发现；默认前端 `web/ggapi`；tag→GHCR 见 SOP §5.1 / GG-003；C2-pre 可走通且修完再审；PR 已开时「继续」不重写代码且不补 push 词；链式发车缺词不补权；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-*` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `docs/ggapi-fork-1.6.0` / `docs/ggapi-fork-1.7.0` |
| GG-003 | infra | `.github/workflows/*`（`pr-check.yml`、`docker-build.yml`、`docker-image-branch.yml`、`release.yml`、`electron-build.yml`） | PR 模板/anti-slop 检查使用 GitHub 托管 `ubuntu-latest`；镜像、分支镜像、手动裸二进制与禁用提示任务继续使用 `org-linux` **Linux amd64**；**tag → GHCR** 仅 `:<version>`（tip 时再加 `:latest`），无冗余 `*-amd64` 正式 tag；`cache-to` **ignore-error**；+ **Release 元数据**；**禁止** Docker Hub `calciumion/new-api`；裸二进制仅 `release.yml` **workflow_dispatch**；electron 构建对本仓禁用 | 公开仓的 PR 门禁无法使用未向公开仓开放且已离线的组织 runner，改用一次性 GitHub 托管 runner 避免永久排队；产品构建仍由组织 runner 承载；部署走镜像；更新检查依赖 `releases/latest`；避免污染上游 Hub；cache 故障不拖垮已 push 镜像 | medium（上游常改 workflow；`sync/upstream-20260713` 冲突保留本仓 GHCR 策略；`pr-check.yml` runner 策略由 #32 单独维护） | 新 PR 的 `pr-quality` 在 `ubuntu-latest` 启动并执行 anti-slop，不再因 `org-linux` 排队/取消；推 fork tag → 仅 `IMAGE:TAG`（+ tip 时 `latest`）+ Release；不默认 bare 二进制；branch 镜像 `branch-*-hash`；上游同步后仍为 `ghcr.io/...` | active | Akuma-real | 2026-07-11 | `chore/org-linux-runner` / `docs/version-tag-scheme` / `chore/tag-ghcr-no-binary` / `fix/ghcr-cache-ignore-error-single-tags` / `sync/upstream-20260713` / `chore/pr-check-hosted-runner` #32 |
