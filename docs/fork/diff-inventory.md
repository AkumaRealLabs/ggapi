...
| GG-007 | feature / patch | `common/{constants,quota_math*}.go`；`model/{affiliate*,db_error*,main,option,redemption*,subscription,task_cas_test,topup,user}.go`；`controller/{affiliate,option,topup,user}.go`；`router/api-router.go`；`web/src/features/{affiliate,wallet,users,system-settings}/**`；`web/src/i18n/locales/**` | 新增一级邀请返佣：在线充值、外部支付订阅与兑换码成功兑换后按默认或邀请人自定义比例结算到 `aff_quota`，以来源唯一账单、订单/兑换事务锁和同事务更新保证幂等；五个充值渠道统一走 `completeTopUp` 事务（行锁 + 状态校验 + 入账 + 返佣同事务），`RechargeEpay` 对齐上游返回 `(alreadyDone, err)` 与类型化错误供 webhook 幂等日志使用；提供用户一次性补绑、管理员改绑/解绑、邀请码与比例管理、返佣明细及全局账单 | 现有固定注册邀请奖励无法覆盖持续充值、订阅与兑换推广，需要可审计、不可重复结算且不影响余额订阅或管理员赠送的独立资金路径 | high（充值/订阅完成事务、兑换事务、用户额度与关系锁、数据库迁移及管理端资金视图） | SQLite/MySQL/PostgreSQL 迁移与唯一索引；五种支付渠道和管理员补单只返佣一次；外部订阅返佣且余额订阅不返；兑换码按兑换时邀请关系以 `RED-<id>` 安全引用返佣且不暴露密钥，重复兑换不重复返佣，返佣写入失败时兑换码/用户额度/邀请人返利全回滚，并发改绑在用户行锁后复核关系，变化则整单回滚并以新事务重试，旧邀请人不会收到返佣；默认/自定义/0%/合规关闭/禁用邀请人及返佣余额溢出按规则跳过且兑换成功；自邀与循环拦截、改绑仅影响后续订单/兑换；账单/订单/用户额度失败全回滚；`aff_quota` 转账后缓存立即可用；用户与管理端来源筛选/展示及七语言键 | active | Akuma-real | 2026-07-14 | `feat/affiliate-commission` / `feat/redemption-affiliate-commission` / `sync/upstream-20260813` |
| GG-008 | patch | `controller/{oauth,user,wechat}.go`；`model/{checkin,errors,payment_method_guard_test,quota_reserve,quota_reserve_test,redemption,subscription,token,token_cache,topup,user,user_auth_cache,user_cache,user_cache_auth_version_test,user_update_test,utils}.go`；`relay/{common/relay_info,mjproxy_handler*}.go`；`service/{billing,billing_session*,funding_source,quota,task_billing*,tiered_settle*}.go`；`web/src/features/keys/components/{api-key-group-cell.tsx,__tests__/api-key-group-cell.test.tsx}` | Auto 跨分组重试切换到更高倍率分组时，在发送上游请求前原子复核并补扣钱包额度；钱包请求不再以信任额度跳过预扣，Midjourney 固定价任务也在发送前完成钱包 CAS 与 Token 预留，只有成功落库的已扣款任务才保留可退款 `Quota`；Realtime usage 增量通过 `BillingSession.Reserve` 原子补足钱包/订阅与 Token 预留，任一资金源失败会补偿另一侧；同步结算、兼容结算与异步任务差额补扣同样复用余额门禁（`DecreaseUserQuotaIfEnough` 现委托上游 `TryReserveUserQuota` 缓存权威预扣并映射 `ErrInsufficientUserQuota`）；`User.Quota` 与 Token 配额不再进入进程内批量 map，`Increase/Decrease*Quota` 即使 `BATCH_UPDATE_ENABLED=true` 也立即落库，Redis 仅作为带写 fence 的可失效缓存；缓存预扣成功后仍执行数据库余额 CAS，拒绝管理员降额后残留的旧缓存继续消费，缓存错误地报告余额不足时也由数据库 CAS 最终裁决已提交的入账或退款；管理员覆盖额度在用户行锁事务内先建立 fence 并删缓存，失效失败则回滚；Token metadata 变更使用长于缓存生命周期且可自动过期的 owner lease，提交后发布权威快照，删除后原子转换为有界 tombstone；exhausted Token 强制刷新时保留数据库错误分类；OAuth/微信绑定目标不存在时返回失败；余额不足返回 `insufficient_user_quota` 且不形成欠费；API key 列表始终显示独立 `Auto` 标识，仅在 `cross_group_retry=true` 时追加 `Cross-group` 状态 | 上游 `rc.23` 的补充预扣与结算差额允许钱包变负，信任额度与 Midjourney 的先请求后扣款路径在并发 CAS 失败时还会漏扣或产生无扣款退款；前端同时无条件显示跨分组状态；进程内 pending map 无法作为多节点共享资金账本，短 TTL 缓存缺失还会导致长请求退款或异步结算永久漏记，钱包与 Token 配额立即落库同时消除批量延迟下缓存过期后从数据库水合到过期余额的问题；缓存失效失败后继续 Token 变更会让旧凭证保持可用 | high（触及请求发送前的计费预扣、同步/异步结算、Midjourney 任务退款、Token 鉴权与钱包/Token 配额持久化；前端显示本身为 low） | `BATCH_UPDATE_ENABLED=true` 时钱包与 Token 扣款/退款仍立即落库，缓存缺失不会漏记且 batch flusher 不会重复应用（`model/utils.go` 无资金配额 batch 类型）；并发请求及高倍率重试只有数据库余额足够的预扣成功，管理员降额后的旧缓存不能把数据库扣成负数，其余在发送上游前返回 `ErrorCodeInsufficientUserQuota`；充值、兑换、签到、返利转余额与余额购订阅提交后均失效旧额度缓存并从数据库水合，旧缓存报告余额不足时由数据库确认已提交授信；Redis 预写 fence 失败时管理员额度覆盖、Token 禁用/删除和批量删除不改变数据库；Token 数据库提交后的缓存发布失败仅记录异常，单个操作返回成功，批量删除返回实际删除数；Token owner lease 长于缓存生命周期、不因超过 10 秒的锁等待而失效，崩溃遗留 lease 会自动过期恢复，单个/批量删除均以有界 tombstone 在保护窗口内阻止旧快照复活；并发 Realtime usage 预留最多一个请求耗尽有限 Token，钱包补偿后两侧余额均不为负；exhausted Token 的数据库故障保持 `ErrDatabase`，已删除用户的 OAuth/微信绑定失败；同步结算与异步任务差额补扣余额不足时保留原余额和结算状态；无 Redis时 SQLite/MySQL/PostgreSQL 均使用条件 `UPDATE` 作为跨节点 CAS，有 Redis 时缓存 Lua 预扣成功后仍以数据库 CAS 确认并立即落库；Auto key 在倍率缺失且跨分组重试关闭时仍显示 `Auto`，`cross_group_retry=true/false` 分别显示/隐藏 `Cross-group` | active | Akuma-real | 2026-08-02 | `sync/upstream-20260802` #39 / `sync/upstream-20260813` |
...
| GG-010 | patch | `relay/{chat_completions_via_responses*,responses_handler.go}` | Chat Completions 与直接 Responses 请求经官方 OpenAI/Azure 路由时移除 `frequency_penalty` / `presence_penalty`；兼容供应商路由仍保留显式值 | 官方 Responses API 不支持 penalty 字段，原样转发会使合法请求被上游拒绝 | low（仅官方 Responses 渠道兼容清理） | OpenAI/Azure 转换结果不含两个 penalty 字段；Advanced Custom 等兼容路由保留显式 `0` 与非零值；`go test ./relay/...` | active | Akuma-real | 2026-08-15 | `sync/upstream-20260813` |
| GG-011 | patch | `controller/midjourney.go`；`relay/{common/billing,mjproxy_handler}.go`；`model/{checkin,errors,midjourney,redemption,subscription,task,token,topup,user,user_update_test}.go`；`service/{billing_session,midjourney,task_billing,task_polling,task_billing_test,task_polling_test}.go` | 所有钱包与 Token 入账统一执行 int32 上限条件更新，覆盖管理员覆盖、新用户初始额度与返佣转余额，Token 可恢复到合法的 `MaxQuota` 边界；Midjourney 和通用异步任务失败退款在单事务内恢复资金、Token、用户/渠道用量并清除持久化 `Quota` 标记；同步计费退款将资金源与 Token 置于同一事务并向调用方返回错误；失败通用任务携带非零 `Quota` 时由调度器仅重试退款而不再请求上游；Midjourney 通知/轮询只更新业务状态字段且禁止终态反转 | 上游 `rc.25` 仅限制充值回调，签到、兑换、退款等路径仍可越过数据库额度域；分步退款或陈旧整行回写会永久多扣、部分退款、恢复已清零标记或把已失败任务改回成功 | high（资金上限、跨表退款事务、终态 CAS 与退款重试） | 钱包/Token 达上限时入账返回类型化错误且余额不变；返佣转余额及新用户额度不能越过上限；Token 退款失败时钱包、Token、用量和 `Quota` 标记均不变，故障解除后重试只退款一次；同步退款任一环节失败时全部回滚；失败任务退款错误解除后由调度器自动重试且不请求上游；陈旧 Midjourney 通知不能恢复退款标记或反转终态；SQLite 单测通过，MySQL/PostgreSQL 使用 `lockForUpdate` 与条件更新保持同一语义 | active | Akuma-real | 2026-08-23 | `sync/upstream-20260823` |
| GG-012 | patch | `web/src/features/channels/{components/dialogs/advanced-custom-editor-dialog.tsx,lib/advanced-custom.ts}`；`web/src/i18n/locales/**` | Advanced Custom 的余额查询校验改用可静态提取的翻译键，补齐展开按钮，并在替换确认框中翻译模板名 | 上游 `rc.25` 的动态校验消息无法被 i18n 工具发现，部分新增文案会回退为英文 | low（仅渠道编辑器文案与校验消息） | 七种 locale 的 missing/extras/untranslated 均为 0；余额查询四类校验、展开按钮及模板确认框随语言切换 | active | Akuma-real | 2026-08-23 | `sync/upstream-20260823` |

**GG-006 会员顺延补充（2026-07-15）：** 同一用户最多一份未到期 `active` 会员；有效期内自助续费、已付款回调或管理员赠送遇到现有会员时创建 `scheduled`，pending 预占名额、最多 3 份并按队尾顺延且不提前切组/写 `PrevUserGroup`。维护任务对额度订阅走通用 expire；会员在 `ActivateDueMembershipSubscriptions` 内同事务 expire→activate，避免中间态用基础分组路由；删除用户导致的孤儿订阅在锁用户失败时清理并跳过，不拖垮整轮 worker。abandoned pending 超过 24h TTL 在 checkout/自助计数前自动过期；有效会员期间创建/更新/启用空分组 Token 被拒，与购买前空分组拦截一致；Token 新增/更新与会员 checkout 共用 user 行锁并在同一事务内校验后写入，并发交错最多一侧成功，不会同时留下会员状态与启用的空分组 Token。`scheduled` 与 active 一并阻止套餐类型切换；SQLite 状态机测试覆盖支付回调竞态、不同会员分组顺序激活、最终回退、队列重排和 worker 幂等；生产串行化依赖 MySQL/PostgreSQL 的 `FOR UPDATE`。套餐 CRUD、赠送、失效/删除和额度重置的业务失败统一返回稳定英文键，官方管理端通过 `skipBusinessError + t(message)` 单点 toast；钱包列表、管理表格、赠送选择器和购买弹窗统一使用 `formatLocalCurrencyAmount`。**2026-08-13 同步补充：** `Token.Update` 只写元数据；`status_only` 走 `UpdateStatusWithMembershipGuard` 且不回写 `remain_quota`，避免覆盖并发预扣。Token metadata 变更使用可续租的 owner lease，提交后发布权威快照；lease 过期后仍删除旧缓存并写入有界 tombstone/recovery fence（配额失效不会缩短该窗口），删除 tombstone 带 TTL 可回收；`CompleteSubscriptionOrder` / `AdminBindSubscription` 未采纳上游"先锁用户行"补丁——`CreateUserSubscriptionFromPlanTx` 内的 plan 行锁已串行化 `MaxPurchasePerUser` 检查，且在 `AdminBindSubscription` 处预锁用户会形成 user→plan 反向锁序（全仓约定 plan→user）；`calcSubscriptionBalanceQuota` 保留 `QuotaFromDecimalChecked` + `ErrSubscriptionPlanPriceRange` 稳定业务键（与上游 `QuotaFromDecimalStrict` 语义等价）。
