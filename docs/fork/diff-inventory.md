...
| 本仓版本策略 | git tag 格式（见 [SOP §5.1](./branch-and-sync-sop.md)） | **`v<上游基线>.N`**，当前基线下首发示例 `v1.0.0-rc.20.1`；基线须为 `origin/main` 祖先；正式 tag **只钉** `origin/main` tip；**禁止**与 upstream 同名 tag；`N` 仅在「基准 upstream 版本」串变化时归 1；尚未发版时勿把示例当已在用版本 |
...
| GG-002 | docs | `.agents/skills/ggapi-fork/`；`docs/fork/README.md` / `AGENTS.md` 入口链接 | `/ggapi-fork` 教练 skill **v1.5.0**（Modes A–I + 可控自升级 + C2-pre + 第三壳教练 + **发版 tag `v<上游>.N`**）；含 org-linux/CI、检查更新 PAT、`gh` 目标仓、i18n zh-TW、release embed 等 | 二开需要命令级教练，且 skill 须能跟进文档/实战而不静默削弱安全规则 | low（独立 skill 目录；入口链接触及 `AGENTS.md`/`docs/fork` 为 medium 旁路） | `/ggapi-fork` 可发现；默认前端路径 `web/ggapi`；tag 规则见 SOP §5.1；提交前 Codex 门禁可走通（或明确跳过）；自升级 L1 可小补、L2/L3 须确认、不自动 commit；文档优先于 skill | active | Akuma-real | 2026-07-11 | `docs/fork-sop` / `docs/ggapi-fork-*` / `docs/version-tag-scheme` |
| GG-003 | infra | `.github/workflows/*`（含 `docker-build.yml`、`docker-image-branch.yml`） | CI 统一 `runs-on.group: org-linux`；仅构建 **Linux amd64**（去掉 arm64 矩阵、macOS/Windows release、Electron 自动构建）；**`docker-build.yml` 关闭 tag 自动触发**；两 Docker workflow 在 image 仍为上游 `calciumion/new-api` 时 **fail-closed**，避免 fork 污染上游 Docker Hub / 覆盖 `:latest` | 使用组织 runner group `org-linux`；本仓只需 Linux x64 产物；禁止误推上游镜像命名空间 | medium（上游常改 workflow；同步时可能恢复 tag 触发） | 新 PR 的 job 在 `org-linux`；`release.yml` 仍可在 tag 上发 GitHub Release；推 tag **不**再自动跑 docker-build；两 Docker workflow 手动 dispatch 在未改 image 前应失败；上游同步后检查未恢复 `push.tags: '*'` + 上游 image | active | Akuma-real | 2026-07-11 | `chore/org-linux-runner` / `docs/version-tag-scheme` |
...
| GG-005 | feature / infra | `web/ggapi/**`；`web/package.json` workspaces；`makefile`（`build-web-ggapi` / `dev-web-ggapi`，`dev`→ggapi）；`Dockerfile` `builder-ggapi`；`.github/workflows/release.yml`（ggapi 前端构建）；`main.go` embed；`common/constants.go` / `embed-file-system.go`；`router/web-router.go`；`setting/system_setting/theme.go`（默认 `ggapi`）；`controller/option.go` 校验；`AGENTS.md` / `docs/fork/*` 第三壳与追 default 说明 | 第三前端壳 `web/ggapi`（自 default 复制起步）；运行时 `theme.frontend=ggapi`；构建/embed/Docker/release 全链路；**功能长期追 `web/default`**，视觉与产品定制落在 ggapi | 避免在 `web/default` 上做永久大改导致上游同步痛苦；独立壳便于皮肤与产品化 | high（三壳源码树 + 多处接线；上游改 theme/embed/Docker/makefile/release 时需重放） | `theme.frontend` 可选 `ggapi\|default\|classic`；默认 ggapi；`make build-web-ggapi` + `go build` embed 通过；Docker/release 三壳产物齐全；同步后 default 新功能有 port 到 ggapi 的 follow-up | active | Akuma-real | 2026-07-11 | `feat/web-ggapi-shell` #7 |
