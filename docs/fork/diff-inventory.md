...
| GG-008 | patch | `web/src/routes/__root.tsx`；`web/src/routes/oauth/$provider.tsx`；`web/src/features/auth/lib/oauth-bind-window*`；`web/src/features/profile/components/tabs/account-bindings-tab.tsx` | OAuth 绑定回调弹窗跳过全局鉴权 bootstrap，通过同源 `postMessage` 将回调交给持有内存 Bearer Token 的 opener；消息以 provider + 高熵 state 认证，不依赖 OAuth 跨站导航后不稳定的 `WindowProxy` 对象身份，并在收到结果前重试投递；登录回调与其他弹窗保持原鉴权初始化 | 绑定弹窗没有独立 Access Token，不应刷新、轮换或清除共享 Refresh Cookie；跨站 OAuth 导航也可能改变窗口代理身份或造成单次消息竞态，任一情况都会打断 opener 绑定响应并最终超时 | high（前端根路由初始化与 OAuth/Session 鉴权流程） | GitHub/Discord/OIDC/LinuxDO/自定义 OAuth 绑定回调不再请求 `/api/user/auth/refresh`，跨站导航后仍由原窗口完成绑定并向实际消息源回传结果；回调消息丢失一次后可重试恢复；错误 provider/state/消息类型不得触发绑定；无 opener 的正常 OAuth 登录仍 bootstrap；非 OAuth 弹窗不得绕过 bootstrap；相关鉴权与 popup 生命周期单测通过 | active | Akuma-real | 2026-07-28 | `fix/oauth-bind-popup-refresh`；`fix/oauth-bind-message-bridge` |
