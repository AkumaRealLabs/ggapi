# 前端开发规范（web/ggapi · 本仓产品壳）

本文档定义前端项目的开发规范与最佳实践，供开发与 AI 助手共同遵循。具体依赖与脚本以 `package.json` 为准。

> **壳角色：** 本目录是 ggapi 默认前端（`theme.frontend=ggapi`）。视觉与产品定制落在这里；**功能追 `web/default`**（上游同步后 port，见 `docs/fork/branch-and-sync-sop.md` §3.5 / 清单 GG-005）。勿把本仓大改只写在 `web/default`。
>
> **默认视觉：** `theme.preset = paper-sketch`（tech-visual-explainer 纸笔手绘，见 `DESIGN.md`）。
---
...
| 工具       | qrcode.react、oxfmt、oxlint、bun test                             |
...
- 工具函数与纯逻辑优先单元测试，测试文件 `*.test.ts`；组件测交互与行为，避免测实现细节。运行器为 **`bun test`**（无需 `test` script，bun 自动发现 `*.test.*`），断言用 `bun:test` 或 `node:test`/`node:assert`——仓库当前用的就是这两者，**未安装 Vitest 或 React Testing Library**，照它们写会因模块解析失败。
...
- 新增或大幅重写测试时优先使用运行器自带的标准断言，避免手写通用断言辅助函数；只有表达项目特定业务不变量时才抽取测试 helper。
