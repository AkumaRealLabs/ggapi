...
export function SystemBrand(props: SystemBrandProps) {
  ...
  const name = status?.system_name || props.defaultName || 'ggapi'
  const version =
  ...
  if (variant === 'inline') {
    return (
      <Link
        ...
        data-slot='sketch-brand'
        className={cn(
          'text-foreground inline-flex h-7 items-center gap-1.5 rounded-md px-1.5 text-sm font-bold transition-colors outline-none select-none',
          'hover:bg-accent focus-visible:ring-ring/40 focus-visible:ring-2'
        )}
      >
        <div
          data-slot='sketch-brand-mark'
          className='flex size-5 items-center justify-center overflow-hidden rounded-md'
        >
          <img
            src={logo}
            alt={t('Logo')}
