...
const codeMirrorTheme = EditorView.theme({
  ...
  '.cm-content': {
    ...
    // Keep full inset on every side. Left must never be 0: when line numbers
    // are off, zero left padding + overflow-hidden + border-radius clips the
    // first glyph of each line (e.g. "from" → "rom").
  ...
  '.cm-line': {
    // Small extra lead-in so mono glyphs never sit under the frame edge.
  ...
  '.cm-scroller': {
    ...
    minHeight: 'var(--code-editor-min-height)',
    overflow: 'auto',
    // Safety inset when gutters are hidden — text stays clear of rounded clip.
...
function CodeMirrorCodeView({
  ...
  return (
    <div
      ...
      className={cn(
        'min-h-(--code-editor-min-height)',
        // Without line-number gutters, keep a host-level inset so the first
        // mono glyph cannot sit under overflow-hidden + border-radius.
        !showLineNumbers && 'px-0.5'
      )}
      ref={editorHostRef}
      role='textbox'
      style={
...
export const CodeBlockFrame = ({
  ...
  <div
    data-slot='sketch-code-block'
    className={cn(
      // overflow-hidden clips to radius; content padding (cm-content) must keep
      // text clear of the edge — do not drop left padding when line numbers are off.
  ...
  >
    {showToolbar && (
      <div
        data-slot='sketch-code-toolbar'
        className='bg-muted/35 border-border/70 flex min-h-10 items-center gap-2 border-b px-2 py-1.5'
      >
...
export const CodeBlock = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ code, language: displayLanguage }}>
      <CodeBlockFrame
        // Horizontal pad on the scroll shell so mono text never sits under the
        // rounded overflow clip (critical when showLineNumbers is false).
        bodyClassName={showLineNumbers ? 'p-0' : 'px-3 py-0'}
