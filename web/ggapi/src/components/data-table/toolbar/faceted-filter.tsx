...
/**
 * Resolve a filter option label for display.
 *
 * Call sites often pass either:
 * - an English i18n key (`'Enabled'`), or
 * - an already-localized string (`t('Enabled')` → `'已启用'`), or
 * - a raw dynamic name (`'svip'`, vendor labels).
 *
 * Always calling `t(label)` double-translates the second case and floods
 * missingKey warnings for Chinese (and other) locales. Only translate when
 * the string is a known catalog key.
 */
...
export function DataTableFacetedFilter<TData, TValue>({
  ...
  if (selectedValues.size === 1 && selectedOptions.length === 1) {
    selectedSummary = resolveOptionLabel(t, i18n, selectedOptions[0].label)
  } else if (selectedValues.size === 2 && selectedOptions.length === 2) {
    selectedSummary = selectedOptions
      .map((option) => resolveOptionLabel(t, i18n, option.label))
      .join(', ')
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      ...
                      title={resolveOptionLabel(t, i18n, option.label)}
                    >
                      {resolveOptionLabel(t, i18n, option.label)}
                    </span>
                    {optionCount != null && (
