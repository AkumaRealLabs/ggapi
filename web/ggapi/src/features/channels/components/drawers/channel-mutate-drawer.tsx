...
function CardHeading({ title, icon }: { title: string; icon?: ReactNode }) {
  return (
    <div data-slot='sketch-drawer-heading' className='flex items-center gap-3'>
      {icon && (
        <span
          data-slot='sketch-drawer-heading-icon'
...
function SubHeading({ title, icon }: { title: string; icon?: ReactNode }) {
  return (
    <div data-slot='sketch-drawer-subhead' className='flex items-center gap-2'>
...
function ChannelEditorNav(props: {
  ...
  return (
    <aside
      data-slot='sketch-drawer-nav'
      className='hidden self-start lg:sticky lg:top-4 lg:z-20 lg:block'
    >
      <div className='flex max-h-[calc(100dvh-12rem)] flex-col gap-3 overflow-y-auto overscroll-contain pr-1'>
        <div
          data-slot='sketch-drawer-inset'
        ...
        <nav
          data-slot='sketch-drawer-nav-list'
    ...
    <>
      <Sheet open={open} onOpenChange={handleOpenChange}>
        ...
        >
          ...
          {!isEditing ? (
            <CreateFlowGuide
              title='How a channel is wired'
              activeStep={(() => {
                // type defaults to 1; advance on real user input
                const hasEndpoint = Boolean(currentBaseUrl?.trim())
                const hasName = Boolean(currentName?.trim())
                const hasKey = Boolean(currentKey?.trim())
                const hasModels = (currentModels?.length ?? 0) > 0
                const hasGroups = (currentGroups?.length ?? 0) > 0
                if (
                  (hasEndpoint || hasName) &&
                  hasKey &&
                  hasModels &&
                ...
                if ((hasEndpoint || hasName) && hasKey) return 2
                if (hasEndpoint || hasName) return 1
                return 0
              })()}
              steps={[
                {
                  label: '① Type & endpoint',
                  hint: 'Pick provider and base URL',
                },
                {
                  label: '② Auth key',
                  hint: 'Upstream credential',
                },
                {
                  label: '③ Models & groups',
                  hint: 'Map what clients can call',
                },
                {
                  label: '④ Save & test',
                  hint: 'Persist then verify',
                },
              ]}
            />
          ) : null}
