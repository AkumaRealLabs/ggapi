...
export function ConsumptionDistributionChart(
  ...
  return (
    <div data-slot='sketch-chart-frame' className='overflow-hidden rounded-lg border'>
      <div
        data-slot='sketch-chart-head'
        className='flex w-full flex-col gap-1.5 border-b px-3 py-2 sm:gap-3 sm:px-5 sm:py-3 lg:flex-row lg:items-center lg:justify-between'
      >
          ...
          {CONSUMPTION_DISTRIBUTION_CHART_OPTIONS.map((item) => {
            ...
            return (
              <button
                key={item.value}
                type='button'
                data-slot='sketch-seg-item'
                data-active={chartType === item.value ? 'true' : undefined}
      ...
      <div data-slot='sketch-chart-body' className='h-[300px] p-1.5 sm:h-96 sm:p-2'>
        {themeReady && spec && (
          <VChart
            key={chartKey}
            spec={{
