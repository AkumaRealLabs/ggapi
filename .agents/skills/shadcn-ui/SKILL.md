...
description: >-
  ...
  web/ggapi (product shell) or web/default UI, shadcn components, or presets.
  Shell-aware: prefer web/ggapi for fork product work (GG-005); use web/default
  only when intentionally editing the upstream-tracking shell. Overview aligns
  with https://ui.shadcn.com/docs/skills.md; full upstream skill text is
  vendored under vendor/shadcn/.
---
...
That installs the skill where the `skills` CLI is available. **This repository** keeps the same intent under `.agents/skills/shadcn-ui/` (overview here + **vendored** upstream docs in [`vendor/shadcn/`](./vendor/shadcn/)) and runs the shadcn CLI from the **active** frontend app root.

### Active shell (required)

| When | App root |
|------|----------|
| Product / paper-sketch / default theme `ggapi` (usual fork work) | `web/ggapi` |
| Upstream-tracking default shell only | `web/default` |

```bash
# Product shell (preferred for ggapi fork UI) — full path each command:
cd web/ggapi && bunx shadcn@latest info --json
# Upstream default only when intentionally editing that tree:
# cd web/default && bunx shadcn@latest info --json
```

Do **not** add components to `web/default` when the task is product-shell UI.
Do not rely on `export WEB_SHELL` across separate agent shell invocations.
...
Run **`shadcn info --json`** from the active shell (`cd web/ggapi && bunx shadcn@latest info --json` (or `web/default`)) for framework, Tailwind version, aliases, base (`radix` | `base`), icon library, installed components, and resolved paths.
...
1. **Project detection** — Applies when `components.json` exists under the active shell (`web/ggapi/components.json` or `web/default/components.json`). Prefer **ggapi** for product work.
2. **Context injection** — Use `shadcn info --json` from that shell as ground truth for imports and APIs.
...
**Workflow:** Prefer this **root** `SKILL.md` for repo paths (`web/ggapi` product shell or `web/default`, Bun). Read **`vendor/shadcn/official-shadcn-ui-workflow.md`** only when you need the complete official component, registry, or preset workflow. Use **`vendor/shadcn/rules/*.md`** when validating concrete markup.
