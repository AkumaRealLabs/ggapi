...
func GetChannel(group string, model string, retry int, requestPath string) (*Channel, error) {
	// Load all enabled abilities for group+model first, then apply Advanced Custom
	// path/model filtering, then choose priority among survivors. Filtering after a
	// single-priority query would drop lower-priority matches when the top channel's
	// routes exclude the requested model (MEMORY_CACHE_ENABLED=false path).
	var abilities []Ability
	...
	// Distinct priorities descending (nil priority treated as 0).
